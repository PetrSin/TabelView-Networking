//
//  PostCourseAlamofierViewController.swift
//  TabelView+Networking
//
//  Created by petar on 30.10.2023.
//
...
class PostCourseAlamofierViewController: UIViewController, UITableViewDataSource {
    ...
    private func createNavigBar(){
        ...
        navigationItem.title = "Post Courses Alamofire"
    }
    ...
    //создаю метод дл активации пост запроса
    func postRequest(){
        //вызываю созданный ранее метод
        AlamofireNetworkRequest.postRequestWithAlamofire(url: postRequestUrl) { courses in
            //через комплишн хендлер передаю полученный из интернета массив
            self.courses = courses
            
             //асинхронно в основную очердь запускю обнавление интерфейса
...
extension PostCourseAlamofierViewController{
    static let idCell = "MyCell"
}

extension PostCourseAlamofierViewController: UITableViewDelegate{
